...
Public Class frmconsultaLetrasRangos
    Private texto As StreamReader
    Private oDataSet As DataSet
    Dim ctaFilas As Integer
    Dim NroPaginasImpresas As Integer = 0
    Private Sub frmconsultaLetrasRangos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.cbxRangoDias.SelectedIndex = 0
    End Sub
    Private Sub cbxRangoDias_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbxRangoDias.SelectedIndexChanged
        ctaFilas = 0
    End Sub
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        ...
        Try
            ...
            swEscritor.WriteLine("Cuotas por vencer dentro de: " & Me.cbxRangoDias.Text & " dias                                fecha reporte: " & DateTime.Today)
            swEscritor.WriteLine("-------------------------------------------------------------------------------------------------------------")
            swEscritor.WriteLine("Núm.Letra      Corr. Mon. Importe  MN    Importe  ME Amortizacion  Saldo     Fecha Emis.    Fecha Venc.      ")
            swEscritor.WriteLine("-------------------------------------------------------------------------------------------------------------")
            ProgressBar1.Maximum = Me.oDataSet.Tables(0).Rows.Count() - 1
            For i As Integer = 0 To Me.oDataSet.Tables(0).Rows.Count - 1
                Connection.Open()
                Dim daLetrasClientes As New SqlDataAdapter("SELECT *FROM letrasClientes where idCliente=" & Me.oDataSet.Tables(0).Rows(i).Item(0) & _
               " and (fecVencimiento<=DATEADD(day," & Me.cbxRangoDias.SelectedItem & ", '" & Me.dtpFechaLiquidacion.Text & "' )) and ((numRecibo=' ' and status=' ') or  (numRecibo<>' ' and status='A'))", Connection)
                ...
                If Me.oDataSet.Tables(1).Rows.Count > 0 Then
                    ...
                    For x As Integer = 0 To Me.oDataSet.Tables(1).Rows.Count - 1
                        ...
                        swEscritor.WriteLine(VisualBasic.Left(Me.oDataSet.Tables(1).Rows(x).Item(7).ToString, 10).PadLeft(15))
                    Next x
                    swEscritor.WriteLine(" ")
        ...
        Try
            ...
            If Not lineaTexto Is Nothing Then
                ...
                If NroPaginasImpresas > 1 Then
                    ...
                    e.Graphics.DrawString("Núm.Letra      Corr. Mon. Importe  MN    Importe  ME Amortizacion  Saldo     Fecha Emis.    Fecha Venc.      ", Fuente, Brushes.Black, 10, 20)
